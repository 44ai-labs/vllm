...
def test_grammar_bitmask_specdec_illegal_draft_does_not_crash():
    # Regression: a grammar-illegal speculative draft token used to trip
    # `assert accepted` in grammar_bitmask. With the clone-walk it must
    # truncate silently and leave the committed grammar untouched.
    tokenizer = AutoTokenizer.from_pretrained(TOKENIZER)
    ...
    # Must not raise (previously: assert accepted), and must not advance or
    # terminate the committed grammar.
